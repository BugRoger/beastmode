...
## Decision
Each phase commits its work to the feature branch at checkpoint. Execute preps versions (plugin.json/marketplace.json/session-start.sh) and L0 proposal. Checkpoint runs retro (while still in worktree) — retro runs only at release, not per-phase — then squash-merges to main, commits, tags, and updates marketplace. Compaction is decoupled from release and runs on-demand via `beastmode compact`. Per-phase commits on the feature branch collapse to one commit on main at release. GitHub release style commit messages.
...
- Retro before merge ensures context changes are included in the release commit
