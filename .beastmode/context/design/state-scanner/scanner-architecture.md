...
## Decision
state-scanner.ts is gutted or deleted. Scanning is composed from store.list() (reads manifests from `.beastmode/state/`) + manifest.deriveNextAction(). No standalone scanner module. Manifest path convention: `state/YYYY-MM-DD-<slug>.manifest.json` (gitignored, CLI-owned).
