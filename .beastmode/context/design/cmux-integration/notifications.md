...
## Decision
Desktop notifications via `cmux notify` fire only on phase failures. Notification policy is fixed — no per-notification verbosity config knobs. Configurable verbosity is deferred.
