...
## Decision
Mutually exclusive `phase/*` labels encode Epic lifecycle: backlog -> design -> plan -> implement -> validate -> release -> done, with cancelled as an additional terminal state. Cancelled epics are closed on GitHub and moved to the Done board column (no custom column). Gates use `gate/awaiting-approval` label + issue comments for pre-code phases, PR reviews for code phases. Transition modes configured in config.yaml.
