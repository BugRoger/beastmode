...
## Decision
Two-tier system: unconditional gates (always enforced, embedded as structural `## N. [GATE|...]` task-runner steps) and configurable gates (human/auto resolved from `.beastmode/config.yaml`). Task runner handles gate detection and substep pruning — gates cannot be bypassed. Config read at each gate — no pre-loading. Phase transitions are externally orchestrated via TypeScript CLI (`beastmode <phase> <slug>`) — Justfile deleted, transition gates removed from config.yaml, checkpoint prints `beastmode <next-phase> <slug>` instead of auto-chaining.
