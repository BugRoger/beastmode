...
## Decision
Three domains: State (feature workflow at `.beastmode/state/`), Context (published knowledge at `.beastmode/context/`), Meta (process knowledge at `.beastmode/meta/`). State tracks where features are. Context documents how to build. Meta captures how to improve with two L2 domains per phase: process (process patterns) and workarounds (beastmode feedback). Meta L3 records are topic-clustered with accumulated observations and confidence tags.
