...
## Decision
Skills (workflow verbs) in `/skills/`, shared utilities in `skills/_shared/`, retro agents in `/agents/`. Each skill has SKILL.md manifest, `phases/` directory (0-3), and optional `references/`. Phase checkpoint files may use blockquote directives before @imports to override shared skill behavior (e.g., skipping the Quick-Exit Check in retro). Directives reference sections by name, not step number, to survive renumbering.
