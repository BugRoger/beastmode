...
describe("buildEnvPrefix", () => {
  ...
  test("omits feature vars when featureSlug is provided without featureId", () => {
    const result = buildEnvPrefix({
      phase: "plan",
      epicId: "bm-abc1",
      epicSlug: "my-epic-abc1",
      featureSlug: "orphan-slug",
    });
    expect(result).not.toContain("BEASTMODE_FEATURE_ID");
    expect(result).not.toContain("BEASTMODE_FEATURE_SLUG");
  });

  test("omits feature vars when featureId is provided without featureSlug", () => {
    const result = buildEnvPrefix({
      phase: "plan",
      epicId: "bm-abc1",
      epicSlug: "my-epic-abc1",
  ...
  test("does not set feature vars to empty string when absent", () => {
    const result = buildEnvPrefix({
      phase: "design",
      epicId: "bm-f3a7",
      epicSlug: "dashboard-redesign-f3a7",
